...
class App extends Component {
  ...
  render() {
    const {score, seconds, activeTabId, mainImage, isGameEnd} = this.state
    const filteredImages = imagesList.filter(
      each => each.category === activeTabId,
    )
    return (
      <div className="bg-container">
        <Navbar navScore={score} navSeconds={seconds} />
        <div className="card">
          {isGameEnd ? (
            <ResultCard resScore={score} onPlayAgain={this.onPlayAgain} />
          ) : (
            <div className="inner-card">
              <img className="main-image" src={mainImage} alt="match" />
              <ul className="tab-list">
                {tabsList.map(each => (
                  <TabItem
                    key={each.tabId}
                    tabItem={each}
                    isActiveTabId={activeTabId}
